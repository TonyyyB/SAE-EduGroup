...
class EleveTable(Table):
    def __init__(self, parent, controller, eleves=None, criteres=None):
        super().__init__(parent, ['Prénom', 'Nom', 'ID'] + [c.get_nom() for c in criteres], enable_scroll_y=True, enable_scroll_x=True)
    ...
    def create_table_from_eleves(self, eleves, criteres):
        ...
        for i, eleve in enumerate(eleves):
            ...
            self.add_row([eleve.prenom, eleve.nom, eleve.num_etudiant] + [critere.to_val(eleve.get_critere(critere)) for critere in criteres])
